...
IntList *getIntListWithRandValues( int amount, bool state )
{
    ...
    {
        for (i = 0; i < amount; i++)
            AddIntValueToList(aList, i+1, "\0");
    }
    else
    {
        for (i = 0; i < amount; i++)
            AddIntValueToList(aList, randomIntFromRange(MIN_VALUE, MAX_VALUE),  "\0");
    }
    return aList;
}
...
int AddIntValueToList(IntList *aList, int anIntValue, char* word)
{
	...
	if (NULL != aList)
	{
		IntNode *theNode = createIntNodeWithValue(anIntValue, word);
...
IntNode *createIntNodeWithValue(int anIntValue, char *word)
{
	...
	if (NULL != theNode)
	{
        ...
        theNode->word = (char*) malloc(sizeof(char)*strlen(word));
        theNode->word = word;
	}
